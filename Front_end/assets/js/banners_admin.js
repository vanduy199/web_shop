...
async function loadAllSlots() {
  ...
  Object.entries(map).forEach(([pos, imgId]) => {
    ...
    if (el) el.dataset.bannerId = b ? b.id : "";
  });
}
...
function renderSelectionThumbs() {
  ...
  bannersList.forEach(b => {
    ...
    div.innerHTML = `<img src="${b.image_url || '../assets/img/placeholder.png'}" alt="" style="width:100px;height:60px;object-fit:cover;display:block;margin:0 auto 4px;"><div class="small text-truncate">${b.title||('ID:'+b.id)}</div>`;
    div.onclick = () => {
      // highlight selection
...
function closeSelectionPanel() {
  ...
  currentSlotPos = null;
  selectedBannerId = null;
  document.getElementById("selection-upload").value = "";
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("selection-apply").addEventListener("click", applySelection);
  document.getElementById("selection-upload-and-apply").addEventListener("click", uploadAndApply);
  document.getElementById("selection-cancel").addEventListener("click", closeSelectionPanel);
});